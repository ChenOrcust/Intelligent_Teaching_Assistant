...
本项目使用阿里云百炼（DashScope）的 API 服务。

#### 获取步骤

1. **访问控制台**
   - 打开 [阿里云百炼控制台 API Key 管理页面](https://bailian.console.aliyun.com/cn-beijing/?tab=model#/api-key)
...
3. **创建 API Key**
   - 在控制台中找到"API Key 管理"或"密钥管理"
   - 点击"创建 API Key"按钮
   - 复制生成的 API Key（注意：API Key 只显示一次，请妥善保存）

4. **新用户免费额度**
   - 🎁 **新注册用户可获得免费额度**
   - 免费额度通常包括一定数量的 API 调用次数
   - 足够进行小规模测试和功能体验
   - 建议先使用免费额度熟悉系统功能

> 💡 **提示**：
> - API Key 是访问服务的凭证，请勿泄露给他人
> - 免费额度用完后，可按需购买付费套餐
> - 建议在测试阶段使用免费额度，确认功能符合需求后再考虑付费
...
```yaml
openai:
  api_key: "your-api-key-here"  # 从阿里云百炼控制台获取
