...
grade_file = r"E:\Projects\Intelligent_Teaching_Assistant\十人小组作业-批改.xlsx"  # 需要修改为实际的十人小组批改文件

# 默认输出文件名（不带时间戳）
default_output_file = (
    r"E:\Projects\Intelligent_Teaching_Assistant\十人小组作业-成绩整理.xlsx"
)
...
def load_student_list(file_path):
    ...
    for row in range(8, ws.max_row + 1):
        ...
        if student_id_str and name_str:
            students.append({"student_id": student_id_str, "name": name_str})
...
def create_output(students, student_to_group, grades_by_chapter, output_path):
    ...
    # 获取所有章节名称并排序
    chapter_names = sorted(grades_by_chapter.keys()) if grades_by_chapter else []
    ...
    for row_idx, student in enumerate(students, start=2):
        ...
        # 填充各章节成绩
        if group_num and grades_by_chapter:
...
def main():
    ...
    # 询问是否使用固定文件名
    use_timestamp = (
        input("是否在输出文件名中添加时间戳？(y/n，默认y): ").strip().lower()
    )
    ...
    if use_timestamp == "n":
        ...
        if Path(output_file).exists():
            overwrite = (
                input(f"文件 {output_file} 已存在，是否覆盖？(y/n): ").strip().lower()
            )
            if overwrite != "y":
                print("操作已取消")
                return
        ...
        output_file = rf"E:\Projects\Intelligent_Teaching_Assistant\十人小组作业-成绩整理_{timestamp}.xlsx"
